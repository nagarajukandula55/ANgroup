...
export default function UsersPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Toast */}
      {toast && (
        <div className="fixed top-6 right-6 z-50 rounded-2xl border border-white/10 bg-zinc-900 px-5 py-3 text-sm text-white shadow-2xl">
          {toast}
        </div>
      )}
      ...
      <div className="flex items-center justify-between">
        <div>
          <p className="text-xs uppercase tracking-[0.4em] text-zinc-500">Admin</p>
      ...
      <div className="flex gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full rounded-xl border border-white/10 bg-white/[0.04] pl-10 pr-4 py-3 text-sm text-white placeholder:text-zinc-600 focus:border-white/20 focus:outline-none"
          />
        </div>
        <button
          onClick={fetchUsers}
          className="rounded-xl border border-white/10 bg-white/[0.04] px-5 py-3 text-sm hover:bg-white/[0.08] transition"
        >
          Search
        </button>
      </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-white/[0.06]">
                <th className="px-6 py-4 text-left text-[10px] uppercase tracking-[0.4em] text-zinc-500 font-normal">User</th>
                <th className="px-6 py-4 text-left text-[10px] uppercase tracking-[0.4em] text-zinc-500 font-normal">Role</th>
                <th className="px-6 py-4 text-left text-[10px] uppercase tracking-[0.4em] text-zinc-500 font-normal">Businesses</th>
                <th className="px-6 py-4 text-right text-[10px] uppercase tracking-[0.4em] text-zinc-500 font-normal">Actions</th>
              </tr>
            </thead>
            <tbody>
              {filtered.map((u) => (
                <tr key={u._id || u.id} className="border-b border-white/[0.04] hover:bg-white/[0.02] transition">
                  <td className="px-6 py-4">
                    <p className="text-sm font-medium text-white">{u.name}</p>
                    <p className="text-xs text-zinc-500 mt-0.5">{u.email}</p>
                  </td>
                  <td className="px-6 py-4">
                    <span className="inline-flex items-center gap-1 rounded-full bg-white/[0.06] px-2.5 py-1 text-xs text-zinc-300">
                      <Shield size={10} />
                      {u.role || "USER"}
                    </span>
                  </td>
                  <td className="px-6 py-4">
                    {(u.businesses || []).length === 0 ? (
                      <span className="text-xs text-zinc-600">None assigned</span>
                    ) : (
                      <div className="flex flex-wrap gap-1.5">
                        {(u.businesses || []).slice(0, 3).map((b) => (
                          <span key={b.businessId} className="inline-flex items-center gap-1 rounded-full bg-white/[0.05] border border-white/[0.07] px-2.5 py-1 text-xs text-zinc-400">
                            <Building2 size={9} />
                            {b.brandName || b.name || b.businessId?.slice(-6)}
                          </span>
                        ))}
                        {(u.businesses || []).length > 3 && (
                          <span className="text-xs text-zinc-600">+{(u.businesses || []).length - 3}</span>
                        )}
                      </div>
                    )}
                  </td>
                  <td className="px-6 py-4 text-right">
                    <button
                      onClick={() => openPanel(u)}
                      className="inline-flex items-center gap-1.5 rounded-xl border border-white/10 bg-white/[0.04] px-3 py-1.5 text-xs hover:bg-white/[0.08] transition"
                    >
                      <UserCog size={12} />
                      Manage
                    </button>
      ...
      {panelOpen && selectedUser && (
        <div className="fixed inset-0 z-50 flex">
          ...
          <div className="w-full max-w-md bg-zinc-950 border-l border-white/10 overflow-y-auto flex flex-col">
            <div className="flex items-center justify-between px-6 py-5 border-b border-white/[0.06]">
              <div>
                <h2 className="text-lg font-semibold">{selectedUser.name}</h2>
                <p className="text-xs text-zinc-500 mt-0.5">{selectedUser.email}</p>
              </div>
              <button onClick={() => setPanelOpen(false)} className="p-2 rounded-xl hover:bg-white/[0.06] transition">
                <X size={16} />
              </button>
            </div>
            ...
            <div className="flex-1 px-6 py-6 space-y-6">
              ...
              {/* Current businesses */}
              <div>
                <p className="text-[10px] uppercase tracking-[0.4em] text-zinc-500 mb-3">Business Access</p>
                {selectedUser.businesses.length === 0 ? (
                  <p className="text-sm text-zinc-600">No businesses assigned</p>
                ) : (
                  <div className="space-y-2">
                    {selectedUser.businesses.map((b) => (
                      <div key={b.businessId} className="flex items-center justify-between rounded-xl border border-white/[0.07] bg-white/[0.03] px-4 py-3">
                        <div>
                          <p className="text-sm font-medium text-white">{b.brandName || b.name || b.businessId}</p>
                          <div className="flex items-center gap-2 mt-1">
                            <span className="text-[10px] text-zinc-500">{b.memberType}</span>
                            {b.isDefaultBusiness && (
                              <span className="text-[10px] text-white/60 bg-white/10 px-1.5 py-0.5 rounded-full">Default</span>
                            )}
                          </div>
                        </div>
                        <button
              ...
              <div className="rounded-2xl border border-white/[0.07] bg-white/[0.02] p-5">
                <p className="text-[10px] uppercase tracking-[0.4em] text-zinc-500 mb-4">Assign Business</p>

                {/* Business select */}
                <div className="mb-3">
                  <label className="block text-xs text-zinc-500 mb-1.5">Business</label>
                  <div className="relative">
                    <select
                      value={assignBiz}
                      onChange={(e) => setAssignBiz(e.target.value)}
                      className="w-full rounded-xl border border-white/10 bg-white/[0.04] px-4 py-2.5 text-sm text-white focus:outline-none focus:border-white/20 appearance-none"
                    >
                ...
                {/* Member type */}
                <div className="mb-3">
                  <label className="block text-xs text-zinc-500 mb-1.5">Role in Business</label>
                ...
                <label className="flex items-center gap-2 mb-4 cursor-pointer">
                  <input
                    ...
                    className="rounded"
                  />
                  <span className="text-xs text-zinc-400">Set as default business</span>
                </label>

                <button
                  onClick={assignBusiness}
                  disabled={!assignBiz || saving}
                  className="w-full rounded-xl bg-white py-2.5 text-sm font-semibold text-black hover:bg-zinc-100 disabled:opacity-40 disabled:cursor-not-allowed transition flex items-center justify-center gap-2"
                >
